...
********************************************************************************
	...
	local 		label 		"table_seasonal_effect_alt"
...
//																								 	Remove all string except for "" if not used.
	...
	local 		cells 		"collabels("Food Insecurity\textsuperscript{1}" "SE") keep(honey_producer_x_honey_season honey_producer month_honey month_lean_new _cons)"
	local 		coef		"coeflabels(honey_producer_x_honey_season  "Honey Producer x Honey Season\textsuperscript{2}" month_honey "Honey Season\textsuperscript{2}" month_lean_new "Lean Season" honey_producer "Honey Producer" _cons "Constant" ) nobaselevels"							// (Optional) Customize all coefficients with coeflabels: (var) "New Name" ...
...
** Interaction terms for IV	
	...
	gen month_lean_new  = 0
	replace month_lean_new = 1  if  months == "july" 
	replace month_lean_new = 1  if  months == "august" 
	
	eststo clear
	eststo: 	reg 	food_insecurity 	honey_producer_x_honey_season 	month_honey month_lean_new 	honey_producer				, $regression
	...
	eststo: 	reg 	food_insecurity 	honey_producer_x_honey_season 	month_honey month_lean_new 	honey_producer female age i.edu_level family_members dependents i.region_survey coffee_experience farm_hectares quintal total_income , $regression
	estadd 		local 	Subjects 	"275"
	estadd		local	Months 		"12"
	estadd		local 	FE 			"NO" 
	...
	eststo: 	reghdfe 	food_insecurity 	honey_producer_x_honey_season 	month_honey month_lean_new 	honey_producer 	i.region_survey		, $regression  absorb(id)
	estadd 		local 	Subjects 	"275"
	estadd		local	Months 		"12"
	estadd 		local 	FE 			"YES"
	estadd 		local 	Regional 	"YES"
...
***********    Model (8): 2nd Stage 2SLS with IV   ******************				
	...
	eststo: 	xtivreg 	food_insecurity ( ///
												...
												= 	iv_share_honey					region_honey_share 	 /// 
													...
													month_honey month_lean_new i.region_survey , fe  $regression
	estadd 		local 	Subjects 	"275"
	estadd		local	Months 		"12"
	estadd		local 	FE 			"YES"
	estadd 		local 	Regional 	"YES"
